...
// --- MOCK DEFINITIONS ---
int mock_scan_complete = -2;
std::vector<std::pair<String, int>> mock_networks;
...
void test_json_streaming_multiple_networks(void) {
    // 1. Setup Mock Data
    mock_networks.push_back({"HomeWiFi", -55}); // 3 bars
    mock_networks.push_back({"GuestWiFi", -85}); // 0 bars
    ...
    TEST_ASSERT_TRUE(output.find("{\"ssid\":\"HomeWiFi\",\"rssi\":-55,\"bars\":3") != std::string::npos);
    TEST_ASSERT_TRUE(output.find("{\"ssid\":\"GuestWiFi\",\"rssi\":-85,\"bars\":0") != std::string::npos);
...
void test_stress_ram_large_dataset(void) {
    ...
    mock_networks.clear();
    for(int i=0; i<200; i++) {
        String ssid = "ExtremelyLongSSIDNameToSendTheUsageUp_" + String(i);
    ...
    TEST_ASSERT_TRUE(request._tempStream->content.find("ExtremelyLongSSIDNameToSendTheUsageUp_199") != std::string::npos);
}
