...
void Application::init() {
  ...
  health.init();
  health.resetHeapWatermark(ESP.getFreeHeap(), ESP.getMaxFreeBlockSize());
...
void Application::handleInitializing() {
  ArduinoOTA.onStart([this]() {
    beginArduinoOtaSession();
    setState(State::UPDATING);
  });
  ArduinoOTA.onProgress([this](unsigned int current, unsigned int total) {
    touchArduinoOtaProgress(current, total);
  });
  ArduinoOTA.onEnd([this]() {
    finishArduinoOtaSession();
...
void Application::handleConnecting() {
  ...
  const WifiManager:REDACTED
  if (wifiState =REDACTED
    setState(State::RUNNING);
  } else if (wifiState =REDACTED
...
void Application::handleRunning() {
  ...
  if (m_heapSampleTimer.hasElapsed()) {
    health.recordHeapSnapshot(ESP.getFreeHeap(), ESP.getMaxFreeBlockSize());
  }
  ...
  if (m_healthCheckTimer.hasElapsed()) {
    ...
    if (score.overall() < 25) {
      ...
      LOG_INFO("HEALTH", F("Score: %u/100 (%s)"), score.overall(), healthGrade);
    }

    // Unsigned subtraction already handles millis() wraparound correctly.
    const unsigned long uptimeMs = millis() - m_bootTime;
...
void Application::beginArduinoOtaSession() {
  m_arduinoOtaActive = REDACTED
  m_arduinoOtaStartedAt = REDACTED
  m_arduinoOtaLastProgressAt = REDACTED
  m_arduinoOtaCurrent = REDACTED
  m_arduinoOtaTotal = REDACTED
  m_arduinoOtaWatchdog.detach();
  m_arduinoOtaWatchdog.attach_ms(1000, [this]() { handleArduinoOtaWatchdog(); });
}

void Application::touchArduinoOtaProgress(size_t current, size_t total) {
  m_arduinoOtaCurrent = REDACTED
  m_arduinoOtaTotal = REDACTED
  m_arduinoOtaLastProgressAt = REDACTED
}

void Application::finishArduinoOtaSession() {
  m_arduinoOtaWatchdog.detach();
  m_arduinoOtaActive = REDACTED
  m_arduinoOtaStartedAt = REDACTED
  m_arduinoOtaLastProgressAt = REDACTED
...
void Application::handleArduinoOtaWatchdog() {
  if (!m_arduinoOtaActive) {
    return;
  }
  const unsigned long now = millis();
  if ((now - m_arduinoOtaLastProgressAt) > AppConstants:REDACTED
      (now - m_arduinoOtaStartedAt) > AppConstants:REDACTED
