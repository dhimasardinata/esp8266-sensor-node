...
constexpr uint16_t SSL_RX_BUF_SIZE = 1024;
...
void setup() {
  ...
  if (crashes >= 4 && crashes <= 7) {
    ...
    BootGuard::clear(); // Breaking the loop: Clear count so we can boot normally once
    delay(100);
    ...
    if (LittleFS.format()) {
      ...
      BootGuard::clear(); // Reset counter after format fix
    } else {
  ...
  static AppServer appServer(server, ws, configManager, sensorManager);
