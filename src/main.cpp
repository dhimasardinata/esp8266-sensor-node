  ...
  struct Runtime {
    ...
    Runtime()
        : server(80),
          ...
          appServer(server, ws, configManager, sensorManager, wifiManager, ntpClient),
          portalServer(server, wifiManager, configManager, ntpClient),
...
void setup() {
  ...
  // Initialize purely volatile RTC cache 
  RtcManager::init();
