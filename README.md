...
```
.
├── data/              # Aset web untuk portal dan terminal
├── docs/              # Dokumentasi pengguna dan referensi internal
├── include/           # Header publik aplikasi dan generated config
├── lib/               # Library inti firmware (NodeCore)
├── scripts/           # Otomasi build, OTA, dan codegen
├── src/               # Entry point dan bootstrap aplikasi
├── test/              # Unit dan integration tests
├── tools/             # Tooling repo, repro, cert, watcher, dan prompt helpers
├── var/               # Artifact/generated output lokal
...
- **Kalibrasi**: Nilai kalibrasi default per-node didefinisikan di `include/config/calibration.h`. Nilai ini dapat diubah saat runtime melalui terminal diagnostik.
...
| `settoken [upload\|ota\|both] <token\|default\|none\|show>` | Yes | Kelola token upload/OTA |
