...
void ConfigManager::getHostname(char* buf, size_t len) const {
  snprintf(buf, len, "gh-%d-node-%d", GH_ID, NODE_ID);
}
...
void ConfigManager::validateAndSanitize() {
  ...
  if (!m_config.ALLOW_INSECURE_HTTPS) {
    if (!isValidHttpsUrl(m_config.DATA_UPLOAD_URL.data()))
      Utils::copy_string(m_config.DATA_UPLOAD_URL, "https://example.com/api/sensor");
    if (!isValidHttpsUrl(m_config.FW_VERSION_CHECK_URL_BASE.data()))
      Utils::copy_string(m_config.FW_VERSION_CHECK_URL_BASE, "https://example.com/api/get-file/");
  }

  sanitizeString(std::span{m_config.AUTH_TOKEN});
  sanitizeString(std::span{m_config.ADMIN_PASSWORD});
  sanitizeString(std::span{m_config.PORTAL_PASSWORD});
}
...
static void loadWifiCredentialsFrom(const char* path, std::span<char> ssid_buf, 
                                    std::span<char> pass_buf, bool* hidden = nullptr) {
  ssid_buf[0] = '\0';
  pass_buf[0] = '\0';
...
bool ConfigManager::saveTempWifiCredentials(std::string_view ssid, std::string_view password, bool hidden) {
  // Use static to keep it off the stack (ESP8266 stack is small)
