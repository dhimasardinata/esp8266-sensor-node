...
namespace {
  ...
  void executeStatusCommand(const CommandContext& context,
    ...
    p.print("  MinFree: %u bytes | MinBlock: %u bytes\n",
    ...
    p.print("\n[TIME] %s (sync: %s ago, src: %s)\n",
            timeStr,
            ntpSince,
            ntpClient.getTimeSourceLabel());
    p.print("[API] Last success: %s ago\n", apiSince);
    p.print("[CACHE] RTC: %u/%u | LittleFS: %lu/%lu B\n",
    ...
    p.print("[CACHE] Emergency: %u/%u | Backpressure: %s\n",
            static_cast<unsigned>(apiClient.getEmergencyQueueDepth()),
            static_cast<unsigned>(apiClient.getEmergencyQueueCapacity()),
            apiClient.isEmergencyBackpressureActive() ? "ON" : "OFF");
    p.print("[WS] Utility enabled: %s\n", Utils::ws_is_enabled() ? "YES" : "NO");
...
bool DiagnosticsTerminal::dispatchCommand(uint32_t cmdHash,
  ...
  switch (cmdHash) {
    ...
    case CmdHash::SETTIME: {
      if (!isAuth)
        return false;
      SetTimeCommand cmd(m_services.ntpClient);
      cmd.execute(ctx);
      return true;
    }
    ...
    case CmdHash::HEAPRESET: {
      ...
      ws_println_client(client, F("[OK] Heap watermark reset"));
      return true;
    }
    case CmdHash::HELP: {
      // Help is public
...
void DiagnosticsTerminal::printHelp(AsyncWebSocketClient* client, bool isAuth) {
  ...
  if (isAuth) {
    ...
    append("REDACTED");
    append("  settime <t> - Set time (epoch or YYYY-MM-DD HH:MM:SS)\n");
    append("REDACTED");
    append("REDACTED");
    flush();
    ...
    append("  heapreset   - Reset heap watermarks\n");
    flush();
  } else {
    append_P(PSTR("REDACTED"));
  }
