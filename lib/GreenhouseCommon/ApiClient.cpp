...
void ApiClient::prepareTlsHeap() {
  ...
  constexpr uint32_t kReleaseWsMarginBlock = 512;
  constexpr uint32_t kReleaseWsMarginTotal = REDACTED
  const bool tlsPressure = (maxBlock < (AppConstants::TLS_MIN_SAFE_BLOCK_SIZE + kReleaseWsMarginBlock) ||
                            totalFree < (AppConstants:REDACTED
  if (!m_qosActive) {
    m_qosBuffers.reset();
    if (tlsPressure) {
      m_httpClient.reset();
  ...
  if (tlsPressure) {
    ...
    LOG_INFO("MEM", F("Released WS cipher under TLS pressure (free=%u, block=%u)"), totalFree, maxBlock);
  }
}
...
bool ApiClient::acquireTlsResources(bool allowInsecure) {
  ...
  // Extra safety margin to avoid BearSSL validator OOM on low/fragmented heap.
...
void ApiClient::releaseTlsResources() {
  ...
  const bool staConnected = (m_wifiManager.getState() == WifiManager::State::CONNECTED_STA);
  const bool tlsPressure =
      (ESP.getMaxFreeBlockSize() < AppConstants::TLS_MIN_SAFE_BLOCK_SIZE || ESP.getFreeHeap() < AppConstants::TLS_MIN_TOTAL_HEAP);
  if (!staConnected || tlsPressure) {
...
void ApiClient::handleTimerTasks() {
  ...
  if (m_uploadState == UploadState::IDLE && m_cacheSendTimer.hasElapsed()) {
    if (m_cacheManager.get_size() > 0 || RtcManager::getCount() > 0) {
...
void ApiClient::handle() {
  ...
  auto maybeRestoreTerminalWs = [&]() {
    if (!m_restoreWsAfterImmediate) {
      return;
    }
    if (m_immediateUploadRequested || m_httpState != HttpState::IDLE) {
      return;
    }
    ...
    if (Utils::ws_set_enabled(true)) {
      ...
      m_restoreWsAfterImmediate = false;
    }
  };
  ...
  if (m_httpState != HttpState::IDLE) {
    ...
    if (m_httpState == HttpState::COMPLETE) {
      ...
      HTTPClient& httpRef = m_httpClient ? *m_httpClient : dummyHttp;
      buildErrorMessage(m_lastResult, httpRef);
...
void ApiClient::requestImmediateUpload(bool restoreWsAfterUpload) {
  ...
  if (restoreWsAfterUpload) {
    m_restoreWsAfterImmediate = true;
