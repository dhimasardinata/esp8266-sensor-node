...
void GetCalibrationCommand::execute(const CommandContext& context) {
  ...
  Utils::ws_printf(context.client,
    ...
    "---------------------------------------------------------\n",
    cfg.TEMP_OFFSET, CompiledDefaults::TEMP_OFFSET,
    cfg.HUMIDITY_OFFSET, CompiledDefaults::HUMIDITY_OFFSET,
    cfg.LUX_SCALING_FACTOR, CompiledDefaults::LUX_SCALING_FACTOR);
}