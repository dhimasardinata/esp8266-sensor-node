...
void SetTimeCommand::execute(const CommandContext& context) {
  ...
  if (*args == '\0') {
    Utils::ws_printf(context.client,
                     "[ERROR] Usage: settime <epoch> OR settime YYYY-MM-DD HH:MM:SS");
    return;
  }
  ...
  if (epoch <= 0) {
    Utils::ws_printf(context.client,
                     "[ERROR] Invalid time. Use epoch or 'YYYY-MM-DD HH:MM:SS'.");
    return;
  }

  m_ntpClient.setManualTime(epoch);
  Utils::ws_printf(context.client, "[SUCCESS] Time set. Epoch=%lu", (unsigned long)epoch);
}
