...
namespace {
  ...
  void ensureLogSpace() {
    ...
    if (f && f.size() > 2048) { f.close(); LittleFS.remove(Paths::CRASH_LOG); }
    else if (f) { f.close(); }
  }
  ...
  const char* getResetReason(uint32_t reason) {
    switch (reason) {
      ...
      case REASON_EXCEPTION_RST: return "Fatal Exception";
      case REASON_SOFT_WDT_RST:  return "Software WDT";
      case REASON_SOFT_RESTART:  return "ESP.restart";
