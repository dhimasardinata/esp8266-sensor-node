...
void AppServer::handleTimeRequest(AsyncWebServerRequest* request) {
  ...
  if (!argName) {
    request->send(400, "application/json", "{\"status\":\"error\",\"msg\":\"missing\"}");
    return;
  }

  String val = request->arg(argName);
  if (val.length() == 0) {
    request->send(400, "application/json", "{\"status\":\"error\",\"msg\":\"empty\"}");
    return;
  }
  ...
  if (endptr && *endptr != '\0') {
    request->send(400, "application/json", "{\"status\":\"error\",\"msg\":\"invalid\"}");
    return;
  }
  ...
  if (epoch <= NTP_VALID_TIMESTAMP_THRESHOLD) {
    request->send(400, "application/json", "{\"status\":\"error\",\"msg\":\"too_old\"}");
    return;
  }

  if (m_ntpClient.getTimeSource() == NtpClient::TimeSource::NTP) {
    request->send(200, "application/json", "{\"status\":\"ignored\"}");
    return;
  }
...
void AppServer::handleStatusRequest(AsyncWebServerRequest* request) {
  ...
  response->printf(
      "{\"firmware\":\"%s\",\"nodeId\":\"%d-%d\",\"freeHeap\":%u,\"minFreeHeap\":%u,\"minMaxBlock\":%u,\"uptime\":\"%luh\","
      "\"ssid\":REDACTED
...
void AppServer::handleFormatFsRequest(AsyncWebServerRequest* request) {
  ...
  request->send(200,
                "application/json",
                "{\"status\":\"ok\",\"message\":\"Filesystem format scheduled. Device will reboot.\"}");
}
