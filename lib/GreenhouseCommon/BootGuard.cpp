...
uint32_t BootGuard::calculateCRC(const RtcData& d) {
  ...
  uint32_t crc = 0;
  crc = Crc32::compute(&d.magic, sizeof(d.magic), crc);
  crc = Crc32::compute(&d.crashCount, sizeof(d.crashCount), crc);
  crc = Crc32::compute(&d.lastReasonRaw, sizeof(d.lastReasonRaw), crc);
  crc = Crc32::compute(&d.lastCrashTime, sizeof(d.lastCrashTime), crc);
  return crc;
}
