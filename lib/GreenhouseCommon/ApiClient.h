...
public:
  ...
  [[nodiscard]] bool isEmergencyBackpressureActive() const {
    return m_emergencyBackpressure;
  }
  // --- NEW: QoS Methods ---
...
private:
  ...
  void logEmergencyQueueState(const char* reason);
  ...
  char m_cloudHost[48] = {0};
  char m_cloudPath[48] = {0};
  char m_edgeHost[48] = {0};
  ...
  UploadRecordSource m_loadedRecordSource = UploadRecordSource::NONE;
  ...
  bool m_forceCloudAfterEdgeFailure = false;  // EDGE fallback chain: gateway(s) -> cloud
  uint8_t m_queuePopFailStreak = 0;
  unsigned long m_queuePopRetryAfter = 0;
  ...
  bool m_restoreWsAfterImmediate = false;
  bool m_isSystemPaused = false;
  uint8_t m_lowMemCounter = 0;
  uint8_t m_rtcFallbackFsFailStreak = 0;
  unsigned long m_rtcFallbackFsRetryAfter = 0;
  std::array<EmergencyRecord, kEmergencyQueueCapacity> m_emergencyQueue{};
  uint8_t m_emergencyHead = 0;
  uint8_t m_emergencyTail = 0;
  uint8_t m_emergencyCount = 0;
  bool m_emergencyBackpressure = false;
  unsigned long m_lastEmergencyLogMs = 0;
