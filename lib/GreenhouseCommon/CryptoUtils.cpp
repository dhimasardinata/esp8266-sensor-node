...
namespace CryptoUtils {
  ...
  AES_CBC_Cipher::~AES_CBC_Cipher() {
    ...
    if (m_workScratch) {
      volatile uint8_t* p = reinterpret_cast<volatile uint8_t*>(m_workScratch.get());
      size_t n = EncryptedPayload::MAX_CIPHERTEXT_SIZE;
      while (n--)
        *p++ = 0;
    }
    if (m_ivScratch) {
      volatile uint8_t* p = reinterpret_cast<volatile uint8_t*>(m_ivScratch.get());
      size_t n = EncryptedPayload::IV_SIZE;
      while (n--)
        *p++ = 0;
    }
  }
  ...
  bool AES_CBC_Cipher::decrypt(
    ...
    if (!ensureScratchBuffers()) {
      LOG_WARN("CRYPTO", "Decrypt skipped: scratch alloc failed");
      unlock();
      return false;
    }
    ...
    if (timeIsSynced) {
      ...
      if (msg_ts > now + window || msg_ts + window < now) {
        LOG_ERROR("CRYPTO",
                  "Time skew failure: Msg=%u, Dev=%u, Diff=%d, Win=%u",
                  msg_ts,
                  now,
                  (int)(msg_ts - now),
                  window);
  ...
  size_t AES_CBC_Cipher::encrypt(std::string_view plaintext, char* out_buf, size_t out_len) const {
    ...
    if (!ensureScratchBuffers()) {
      LOG_WARN("CRYPTO", "Encrypt skipped: scratch alloc failed");
      unlock();
      return 0;
    }
    uint8_t* work_buf = m_workScratch.get();
