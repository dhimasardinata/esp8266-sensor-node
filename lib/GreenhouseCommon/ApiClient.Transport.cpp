...
namespace {
  ...
  static constexpr char kTaAtomicHost[] = "ta.example.com";

  struct EdgeGatewayTargets {
    const char* primaryMdns;
    const char* primaryIp;
    const char* secondaryMdns;
  ...
  inline EdgeGatewayTargets resolveEdgeGatewayTargets() {
    ...
    const char* primaryMdns = preferGh1 ? kGatewayGh1 : kGatewayGh2;
    const char* secondaryMdns = preferGh1 ? kGatewayGh2 : kGatewayGh1;
    ...
    return {primaryMdns, primaryIp, secondaryMdns, secondaryIp};
  }
  ...
  inline bool host_matches_domain(const char* host, const char* domain) {
    ...
    const size_t host_len = static_cast<size_t>(end - host);
    const size_t domain_len = strlen(domain);
    return host_len == domain_len && strncasecmp(host, domain, domain_len) == 0;
  }

  inline const char* select_token_for_host(ConfigManager& configManager, const char* host) {
    if (host_matches_domain(host, kTaAtomicHost) && FACTORY_OTA_TOKEN[0] != REDACTED
      return FACTORY_OTA_TOKEN;
    }
    return configManager.getAuthToken();
  }

  inline bool build_auth_header_for_host(char* out, size_t out_len, ConfigManager& configManager, const char* host) {
    if (!out || out_len == 0) {
      return false;
    }
...
void ApiClient::setupHttpHeaders(HTTPClient& http, const AppConfig& cfg) {
  ...
  char authBuffer[kBearerHeaderBufferLen];
  if (build_auth_header_for_host(authBuffer, sizeof(authBuffer), m_configManager, m_cloudHost)) {
...
bool ApiClient::executeQosSample(HTTPClient& http,
  ...
  if (http.begin(m_secureClient, url)) {
    char host[48] = {0};
    char path[8] = {0};
    resolveCloudTarget(url, host, sizeof(host), path, sizeof(path));

    char authBuf[kBearerHeaderBufferLen];
    if (build_auth_header_for_host(authBuf, sizeof(authBuf), m_configManager, host)) {
...
void ApiClient::handleStateConnecting(const AppConfig& cfg) {
  ...
  EdgeGatewayTargets edgeTargets = {nullptr, nullptr, nullptr, nullptr};
  uint16_t port = 0;
  m_edgeHost[0] = '\0';

  if (isEdge) {
    edgeTargets = resolveEdgeGatewayTargets();
  ...
  if (isEdge) {
    ...
    for (size_t i = 0; i < (sizeof(candidates) / sizeof(candidates[0])) && !connected; ++i) {
      ...
      if (i > 0) {
        LOG_WARN("API", F("Edge fallback -> %s"), labels[i]);
      }
      connected = m_activeClient->connect(candidate, port);
      if (connected) {
        host = candidate;
      }
    }
    ...
    connected = m_activeClient->connect(host, port);
  }

  if (connected) {
    if (isEdge) {
      copy_trunc(m_edgeHost, sizeof(m_edgeHost), host);
    }
...
void ApiClient::handleStateSending(const AppConfig& cfg) {
  ...
  EdgeGatewayTargets edgeTargets = resolveEdgeGatewayTargets();
  const char* host = m_targetIsEdge
                         ? ((m_edgeHost[0] != '\0')
                                ? m_edgeHost
                                : edgeTargets.primaryMdns)
                         : m_cloudHost;
...
UploadResult ApiClient::performImmediateUpload() {
  ...
  if (m_queuePopRetryAfter != 0 &&
    ...
    copy_trunc(result.message, sizeof(result.message), "Queue pop cooldown", sizeof("Queue pop cooldown") - 1);
    return result;
  }
  ...
  if (m_queuePopFailStreak > 0 && m_loadedRecordSource != UploadRecordSource::NONE) {
    ...
    if (popLoadedRecord()) {
      ...
      broadcastEncrypted("[QUEUE] Recovery OK (immediate): pop succeeded.");
      copy_trunc(result.message, sizeof(result.message), "Queue pop recovered", sizeof("Queue pop recovered") - 1);
    } else {
      applyQueuePopFailureCooldown(cfg, sourceText);
      copy_trunc(result.message, sizeof(result.message), "Queue pop retry", sizeof("Queue pop retry") - 1);
    }
    return result;
  }
  ...
  if (loadStatus != UploadRecordLoad::READY || record_len == 0) {
    if (loadStatus == UploadRecordLoad::RETRY) {
      copy_trunc(result.message, sizeof(result.message), "Queue recovery", sizeof("Queue recovery") - 1);
      return result;
    }
    if (loadStatus == UploadRecordLoad::EMPTY) {
      copy_trunc(result.message, sizeof(result.message), "Queue empty", sizeof("Queue empty") - 1);
      return result;
    }
    copy_trunc(result.message, sizeof(result.message), "Queue read failed", sizeof("Queue read failed") - 1);
    return result;
  }
  ...
  if (m_uploadMode == UploadMode::AUTO) {
    if (m_immediatePollReady) {
      ...
      if (gwMode != 1) {
        ...
        m_httpClient.reset();
        LOG_WARN("MEM", F("Immediate deferred: warmup before HTTPS path (gwMode=%d)"), gwMode);
  ...
  if (isTargetEdge) {
    ...
    {
      char msg[96];
      int n = snprintf(msg, sizeof(msg), "[UPLOAD] Immediate EDGE send from %s (%u B)", sourceText, static_cast<unsigned>(record_len));
      if (n > 0) {
    ...
    if (encLen > 0) {
      ...
      if (!m_httpClient) {
        result.success = false;
        copy_trunc(result.message, sizeof(result.message), "HTTP alloc fail");
      } else {
        EdgeGatewayTargets edgeTargets = resolveEdgeGatewayTargets();
        const char* candidates[] = {
            edgeTargets.primaryMdns,
        ...
        for (size_t i = 0; i < (sizeof(candidates) / sizeof(candidates[0])) && !result.success; ++i) {
          const char* candidate = candidates[i];
          if (!candidate || candidate[0] == '\0') {
            continue;
          }
          ...
          if (i > 0) {
            LOG_WARN("API", F("Immediate EDGE fallback -> %s"), labels[i]);
          }

          char url[96] = {0};
          int urlLen = snprintf(url, sizeof(url), "http://%s/api/data", candidate);
          if (urlLen <= 0 || static_cast<size_t>(urlLen) >= sizeof(url)) {
            continue;
          }

          m_httpClient->setReuse(false);
          if (!m_httpClient->begin(m_plainClient, url)) {
            continue;
          }
        ...
        if (result.success) {
          copy_trunc(result.message, sizeof(result.message), "OK (Edge)");
        } else {
          copy_trunc(result.message, sizeof(result.message), "Fail (Edge)");
          ...
          if (cloudLoad == UploadRecordLoad::READY && cloud_len > 0) {
            const char* fallbackSourceText =
                (m_loadedRecordSource == UploadRecordSource::RTC) ? "RTC"
                : (m_loadedRecordSource == UploadRecordSource::LITTLEFS) ? "LittleFS"
                                                                          : "Unknown";
            char cloudMsg[104];
            int cn = snprintf(cloudMsg,
                              sizeof(cloudMsg),
                              "[UPLOAD] Immediate HTTPS fallback from %s (%u B)",
                              fallbackSourceText,
    ...
    {
      char msg[100];
      int n = snprintf(msg, sizeof(msg), "[UPLOAD] Immediate HTTPS send from %s (%u B)", sourceText, static_cast<unsigned>(record_len));
      if (n > 0) {
        const size_t len = static_cast<size_t>(std::min<int>(n, static_cast<int>(sizeof(msg) - 1)));
        broadcastEncrypted(std::string_view(msg, len));
      }
...
UploadResult ApiClient::performSingleUpload(const char* payload, size_t length, bool allowInsecure) {
  ...
  if (hasAuthHeader) {
    m_secureClient.print(F("Authorization: REDACTED
    m_secureClient.print(authBuf);
    m_secureClient.print(F("\r\n"));
  }
  m_secureClient.print(F("Content-Length: "));
