...
namespace Utils {
  ...
  void ws_send_encrypted(AsyncWebSocketClient* client, const char* plainText) {
    ...
    // Use shared cipher to avoid duplicate BearSSL contexts
    auto encrypted_payload = CryptoUtils::getSharedCipher().encrypt(plainText);
  ...
  time_t parse_http_date(const char* dateStr) {
    if (!dateStr || strnlen(dateStr, 30) < 10) return 0;

    // Expected format: "Wed, 21 Oct 2015 07:28:00 GMT"
    int day, year, h, m, s;
    char monthStr[5]; // Increased from 4 for safety
    char weekday[5];  // Increased from 4 for safety
