...
void LoginCommand::execute(const CommandContext& context) {
  ...
  if (m_authManager.isClientLockedOut(clientId)) {
    Utils::ws_printf_P(context.client, PSTR("[ERROR] Too many attempts. Try later."));
    return;
  }
  ...
  if (argBuffer[0] == '\0') {
    Utils::ws_printf_P(context.client, PSTR("[ERROR] Usage: login <password>"));
    return;
  }
  ...
  if (valid) {
    ...
    Utils::ws_printf_P(context.client, PSTR("Authentication successful."));
  } else {
    m_authManager.recordFailedLogin(clientId);
    Utils::ws_printf_P(context.client, PSTR("[ERROR] Authentication failed."));
  }
}
