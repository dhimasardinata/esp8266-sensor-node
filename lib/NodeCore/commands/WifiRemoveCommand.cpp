...
void WifiRemoveCommand:REDACTED
  if (!ctx.args || strnlen(ctx.args, 1) == 0) {
    Utils::ws_printf_P(ctx.client, PSTR("[ERROR] Usage: wifiremove <ssid>\n"));
    return;
  }
  ...
  if ((p && strcmp(ssid, p->ssid) =REDACTED
    Utils::ws_printf_P(ctx.client, PSTR("[ERROR] Cannot remove built-in network '%s'.\n"), ssid);
    return;
  }

  Utils::ws_printf_P(ctx.client,
                     m_wifiManager.removeUserCredential(ssid) ? PSTR("[OK] Removed: REDACTED
                                                              : PSTR("[ERROR] '%s' not found.\n"),
                     ssid);
}
