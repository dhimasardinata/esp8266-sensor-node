...
namespace Utils {
  ...
  const char* redact(const char* input, std::span<char> out, size_t keep_head, size_t keep_tail) {
    if (out.empty())
      return "";
    if (!input) {
      strncpy_P(out.data(), PSTR("<null>"), out.size() - 1);
      out[out.size() - 1] = '\0';
      return out.data();
    }
  ...
  void ws_send_encrypted(AsyncWebSocketClient* client, std::string_view plainText) {
    ...
    auto send_chunks = [&](const char* data, size_t len) {
      const size_t maxChunk = CryptoUtils::MAX_PLAINTEXT_SIZE;
      auto wait_until_sendable = [&]() {
        constexpr uint8_t kMaxWaitYields = 8;
        for (uint8_t i = 0; i < kMaxWaitYields; ++i) {
          if (client->canSend()) {
            return true;
          }
          // ws_send_encrypted() is used from both CONT and SYS contexts
          // (e.g. websocket connect callbacks), so plain yield() would panic.
          esp_yield();
        }
      ...
      while (offset < len) {
        ...
        if (!client->text(state->buf.data(), written)) {
          esp_yield();
          break;
        }
        offset += chunk_len;
  ...
  void ws_send_encrypted(AsyncWebSocketClient* client, const __FlashStringHelper* plainText) {
    if (!plainText)
      return;

    PGM_P flashText = reinterpret_cast<PGM_P>(plainText);
    size_t totalLen = REDACTED
    if (totalLen =REDACTED
      return;

    std::array<char, CryptoUtils::MAX_PLAINTEXT_SIZE> plainBuf{};
    size_t offset = 0;
    while (offset < totalLen) {
      size_t chunkLen = totalLen - offset;
      if (chunkLen > plainBuf.size())
        chunkLen = plainBuf.size();
      memcpy_P(plainBuf.data(), flashText + offset, chunkLen);
      ws_send_encrypted(client, std::string_view(plainBuf.data(), chunkLen));
      offset += chunkLen;
    }
  }
  ...
  void ws_printf_P(AsyncWebSocketClient* client, PGM_P fmt, ...) {
    WsState* state = ensure_ws_state();
    if (!state)
      return;
    ...
    va_start(args, fmt);
    int len = vsnprintf_P(stackBuf, sizeof(stackBuf), fmt, args);
    va_end(args);
    ...
    if (state->printfCap < needed + 1) {
      std::unique_ptr<char[]> newBuf(new (std::nothrow) char[needed + 1]);
      if (!newBuf) {
        LOG_ERROR("UTILS", F("OOM in ws_printf_P"));
        {
          InterruptGuard guard;
          state->printfBusy = false;
        }
    ...
    va_start(args, fmt);
    vsnprintf_P(state->printfBuf.get(), state->printfCap, fmt, args);
    va_end(args);
  ...
  static int get_month_index(const char* m) {
    static PGM_P months[] = {
        PSTR("Jan"), PSTR("Feb"), PSTR("Mar"), PSTR("Apr"), PSTR("May"), PSTR("Jun"),
        PSTR("Jul"), PSTR("Aug"), PSTR("Sep"), PSTR("Oct"), PSTR("Nov"), PSTR("Dec")};
    for (int i = 0; i < 12; ++i) {
      if (strncmp_P(m, months[i], 3) == 0) {
        return i;
      }
    }
