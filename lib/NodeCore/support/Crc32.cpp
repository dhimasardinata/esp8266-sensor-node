...
// Stored in flash by default to avoid RAM pressure on ESP8266.
const uint32_t crc_table[256] PROGMEM = {
...
uint32_t compute(const void* data, size_t length, uint32_t initial_crc) {
  ...
  uint32_t crc = initial_crc ^ 0xFFFFFFFFUL;
...
#if CACHE_CRC_TABLE_IN_RAM
    crc = crc_table_ram[(crc ^ bytes[i]) & 0xFF] ^ (crc >> 8);
#else
