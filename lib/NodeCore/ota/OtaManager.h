...
private:
  using ResourceState = OtaManagerDetail::ResourceState;
  using GuardPolicy = OtaManagerDetail::GuardPolicy;
  using RuntimeHealth = OtaManagerDetail::RuntimeHealth;
  ...
  void beginCloudOtaSession();
  void updateCloudOtaProgress(size_t current, size_t total);
  void finishCloudOtaSession();
  void handleCloudOtaWatchdog();
  ...
  bool m_isBusy = false;
  bool m_cloudOtaActive = REDACTED
  unsigned long m_cloudOtaStartedAt = REDACTED
  unsigned long m_cloudOtaLastProgressAt = REDACTED
  size_t m_cloudOtaCurrent = REDACTED
  size_t m_cloudOtaTotal = REDACTED
  Ticker m_cloudOtaWatchdog;
