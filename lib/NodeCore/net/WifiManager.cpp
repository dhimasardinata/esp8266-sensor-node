...
namespace {
  ...
  constexpr unsigned long SCAN_SNAPSHOT_IDLE_MS = 60000;
...
void WifiManager:REDACTED
  ...
  if (!m_configManager) {
    LOG_ERROR("WIFI", F("net/WifiManager:REDACTED
    return;
  }
...
bool WifiManager:REDACTED
  return m_scanInProgress || m_roamingScanInProgress || m_liteScanInProgress || m_forcePortalScanInProgress ||
         m_snapshotScanRequested || m_snapshotScanInProgress;
}
...
void WifiManager:REDACTED
  ...
  m_scanResultCap = 0;
  m_scanResultCount = 0;
  m_hasScanSnapshot = false;
  m_scanSnapshotLastAccess = 0;
}

uint8_t WifiManager:REDACTED
  if (!out || max == 0 || m_scanResultCount == 0 || !m_scanResults)
    return 0;
  m_scanSnapshotLastAccess = millis();
...
bool WifiManager:REDACTED
  if (m_hasScanSnapshot) {
    m_scanSnapshotLastAccess = millis();
  }
  return m_hasScanSnapshot;
}
...
WifiManager:REDACTED
  if (m_wifiState != REDACTED
    return ScanRequestStatus::UNAVAILABLE;
  }
  if (isScanBusy()) {
    return ScanRequestStatus::BUSY;
  }

  const uint32_t freeHeap = ESP.getFreeHeap();
  const uint32_t freeBlock = ESP.getMaxFreeBlockSize();
  if (freeHeap < SCAN_MIN_HEAP || freeBlock < SCAN_MIN_BLOCK) {
    return ScanRequestStatus::LOW_MEMORY;
  }

  m_snapshotScanRequested = true;
  return ScanRequestStatus::QUEUED;
}
...
void WifiManager:REDACTED
  ...
  if (scanCount == 0) {
    m_hasScanSnapshot = true;
    m_scanSnapshotLastAccess = now;
    return;
  }
  if (!ensureScanBuffer()) {
    return;
...
void WifiManager:REDACTED
  if (WiFi.status() != REDACTED
    ...
    m_snapshotScanRequested = false;
    m_snapshotScanInProgress = false;
  ...
  if (!isScanBusy()) {
    if (!m_hasScanSnapshot) {
      releaseScanBuffer();
    } else if (m_scanSnapshotLastAccess != 0 &&
               static_cast<int32_t>(millis() - m_scanSnapshotLastAccess) >=
                   static_cast<int32_t>(SCAN_SNAPSHOT_IDLE_MS)) {
      releaseScanBuffer();
    }
  }
  ...
  if (m_snapshotScanRequested) {
    const uint32_t freeHeap = ESP.getFreeHeap();
    const uint32_t freeBlock = ESP.getMaxFreeBlockSize();
    if (freeHeap < SCAN_MIN_HEAP || freeBlock < SCAN_MIN_BLOCK) {
      LOG_WARN("REDACTED",
               F("Requested scan skipped (low heap: %u, block %u)"),
               freeHeap,
               freeBlock);
      m_snapshotScanRequested = false;
      m_hasScanSnapshot = false;
    ...
    WiFi.scanNetworksAsync(
        [](int /*result*/) {
          // Result is processed in the next handle() loop.
        },
        false);
    m_disconnectWdTimer.reset();
    return;
...
void WifiManager:REDACTED
  ...
  auto savedCredentials = m_credentialStore.getSavedCredentials();
...
void WifiManager:REDACTED
  ...
  m_hasScanSnapshot = (status == OK) || (m_scanResultCount > 0);
  finalizeLiteScan();
}
