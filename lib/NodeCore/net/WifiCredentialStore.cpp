...
#define ENABLE_BUILTIN_WIFI_CREDENTIALS 1
  ...
  PGM_P availability_label_P(bool available) {
    return available ? PSTR("OK") : PSTR("nm");
  }

  void restore_runtime_state(WifiCredential& cred) {
    if (cred.isEmpty()) {
      return;
    }
    cred.setAvailable(cred.isHidden());
    cred.lastRssi = cred.isHidden() ? -95 : -100;
  }

  void strip_transient_state(WifiCredential& cred) {
    if (cred.isEmpty()) {
      return;
    }
    cred.setAvailable(false);
    cred.lastRssi = cred.isHidden() ? -95 : -100;
  }
}  // namespace
...
void WifiCredentialStore:REDACTED
  ...
  char primaryStatus[3];
  char secondaryStatus[3];
  strncpy_P(primaryStatus, availability_label_P(m_primaryGH.isAvailable()), sizeof(primaryStatus) - 1);
  strncpy_P(secondaryStatus, availability_label_P(m_secondaryGH.isAvailable()), sizeof(secondaryStatus) - 1);
  primaryStatus[sizeof(primaryStatus) - 1] = '\0';
  secondaryStatus[sizeof(secondaryStatus) - 1] = '\0';
