...
void AppServer::handle() {
  if (m_isRunning) {
    ...
    if (m_formatFsPending) {
      m_formatFsPending = false;
      LOG_WARN("APP", F("Format FS requested. Rebooting to safe reset mode..."));
    ...
    if (m_webOtaActive) {
      ...
      if (now - m_webOtaLastChunkAt > AppConstants:REDACTED
        abortWebOtaSession(F("REDACTED"));
      } else if (now - m_webOtaStartedAt > AppConstants:REDACTED
        abortWebOtaSession(F("REDACTED"));
      }
    }
  }
...
void AppServer::startWebOtaSession(size_t totalSize) {
  m_webOtaActive = REDACTED
  m_webOtaStartedAt = REDACTED
  m_webOtaLastChunkAt = REDACTED
  m_webOtaExpectedSize = REDACTED
  m_webOtaReceivedSize = REDACTED
}

void AppServer::touchWebOtaSession(size_t chunkLen) {
  if (!m_webOtaActive) {
    return;
  }
  m_webOtaLastChunkAt = REDACTED
...
void AppServer::finishWebOtaSession() {
  m_webOtaActive = REDACTED
  m_webOtaStartedAt = REDACTED
  m_webOtaLastChunkAt = REDACTED
  m_webOtaExpectedSize = REDACTED
...
void AppServer::abortWebOtaSession(const __FlashStringHelper* reason) {
  if (!m_webOtaActive) {
    return;
  }

  LOG_ERROR("REDACTED",
            F("Aborting stalled OTA (%S). received=REDACTED
            reason,
            static_cast<unsigned>(m_webOtaReceivedSize),
            static_cast<unsigned>(m_webOtaExpectedSize),
