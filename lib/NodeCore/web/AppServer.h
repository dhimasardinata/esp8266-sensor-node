...
public:
  AppServer(AsyncWebServer& server,
            AsyncWebSocket& ws,
...
private:
  ...
  void handleFormatFsRequest(class AsyncWebServerRequest* request);
  void handleTimeRequest(class AsyncWebServerRequest* request);
  ...
  void startWebOtaSession(size_t totalSize);
  void touchWebOtaSession(size_t chunkLen);
  void finishWebOtaSession();
  void abortWebOtaSession(const __FlashStringHelper* reason);
  ...
  bool m_formatFsPending = false;
  bool m_webOtaActive = REDACTED
  unsigned long m_webOtaStartedAt = REDACTED
  unsigned long m_webOtaLastChunkAt = REDACTED
  size_t m_webOtaExpectedSize = REDACTED
  size_t m_webOtaReceivedSize = REDACTED
