...
namespace CmdHash {
  ...
  constexpr uint32_t NETCONFIG = CompileTimeUtils::ct_hash("netconfig");
  ...
  constexpr uint32_t SETGATEWAY = CompileTimeUtils::ct_hash("setgateway");
  constexpr uint32_t SETTOKEN = REDACTED
  constexpr uint32_t SETURL = CompileTimeUtils::ct_hash("seturl");
  constexpr uint32_t SETTIME = CompileTimeUtils::ct_hash("settime");
  ...
  constexpr uint32_t UPLINK = CompileTimeUtils::ct_hash("uplink");
  constexpr uint32_t HELP = CompileTimeUtils::ct_hash("help");
  constexpr uint32_t FORCEOTAINSECURE = REDACTED
  constexpr uint32_t HEAPRESET = CompileTimeUtils::ct_hash("heapreset");
}  // namespace CmdHash
...
public:
  ...
  [[nodiscard]] bool queueClientOutput(uint32_t clientId, std:REDACTED
...
private:
  ...
  void flushPendingInitFrames();
  void flushPendingClientOutput();
  void sendInitFrame(AsyncWebSocketClient* client);
  ...
  struct ClientState {
    ...
    bool pendingInitFrame = false;
    std::unique_ptr<char[]> pendingOutput;
    size_t pendingOutputLen = 0;
