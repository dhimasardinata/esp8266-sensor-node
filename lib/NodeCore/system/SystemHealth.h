...
namespace SystemHealth {
  ...
  struct HealthScore {
    ...
    void copyGrade(char* out, size_t out_len) const {
      if (!out || out_len == 0) {
        return;
      }
      uint8_t score = overall();
      PGM_P grade = PSTR("CRITICAL");
      if (score >= 90)
        grade = PSTR("EXCELLENT");
      else if (score >= 75)
        grade = PSTR("GOOD");
      else if (score >= 50)
        grade = PSTR("FAIR");
  ...
  public:
    ...
    void init() {
      ...
      m_minFreeHeap = 0xFFFFFFFFu;
      m_minMaxBlock = 0xFFFFFFFFu;
      m_lastFreeHeap = 0;
      m_lastMaxBlock = 0;
    }
    ...
    void recordHeapSnapshot(uint32_t freeHeap, uint32_t maxBlock) {
      m_lastFreeHeap = freeHeap;
      m_lastMaxBlock = maxBlock;
      if (freeHeap < m_minFreeHeap)
        m_minFreeHeap = freeHeap;
      if (maxBlock < m_minMaxBlock)
        m_minMaxBlock = maxBlock;
    }

    void resetHeapWatermark(uint32_t freeHeap, uint32_t maxBlock) {
      m_minFreeHeap = freeHeap;
      m_minMaxBlock = maxBlock;
      m_lastFreeHeap = freeHeap;
      m_lastMaxBlock = maxBlock;
    ...
    uint32_t getMinFreeHeap() const {
      return (m_minFreeHeap == 0xFFFFFFFFu) ? 0 : m_minFreeHeap;
    }
    uint32_t getMinMaxBlock() const {
      return (m_minMaxBlock == 0xFFFFFFFFu) ? 0 : m_minMaxBlock;
    }
  ...
  private:
    ...
    uint32_t m_minFreeHeap = 0xFFFFFFFFu;
    uint32_t m_minMaxBlock = 0xFFFFFFFFu;
    uint32_t m_lastFreeHeap = 0;
    uint32_t m_lastMaxBlock = 0;
