...
// On-disk config layout for config v2/v3.
...
// On-disk config layout for config v4.
...
// On-disk config layout for config v5+.
struct StoredConfig {
  uint32_t DATA_UPLOAD_INTERVAL_MS;
  uint32_t SENSOR_SAMPLE_INTERVAL_MS;
  uint32_t CACHE_SEND_INTERVAL_MS;
...
#define FACTORY_OTA_TOKEN "REDACTED"
  ...
  bool isValidHttpsUrl(const char* url) {
    ...
    return len >= 12 && len < MAX_URL_LEN && strncmp_P(url, PSTR("https://"), 8) == 0;
  }
...
void ConfigManager::applyDefaults() {
  ...
  validateAndSanitize();
  const bool stringsChanged = validateAndSanitizeStrings(*m_strings);
  if (stringsChanged) {
    LOG_WARN("CONFIG", F("Defaults sanitized"));
  }

  const ConfigStatus saveStatus = save();
  if (saveStatus != ConfigStatus::OK) {
...
ConfigStatus ConfigManager::loadFromFile(const char* path) {
  ...
  if (header_v2.version == 1) {
    ...
    strings.OTA_AUTH_TOKEN = REDACTED
    strings.DATA_UPLOAD_URL = config_v1.DATA_UPLOAD_URL;
    strings.FW_VERSION_CHECK_URL_BASE = config_v1.FW_VERSION_CHECK_URL_BASE;
    strings.GATEWAY_HOST_GH1 = FactoryDefaults::STRINGS.GATEWAY_HOST_GH1;
    strings.GATEWAY_HOST_GH2 = FactoryDefaults::STRINGS.GATEWAY_HOST_GH2;
    strings.GATEWAY_IP_GH1 = FactoryDefaults::STRINGS.GATEWAY_IP_GH1;
    strings.GATEWAY_IP_GH2 = FactoryDefaults::STRINGS.GATEWAY_IP_GH2;
  ...
  if (header_v2.version >= 5) {
    ...
    if (header_v2.version >= 3) {
      uint32_t actual_crc = Crc32::compute(&stored, sizeof(StoredConfig));
    ...
    strings.GATEWAY_HOST_GH1 = stored.GATEWAY_HOST_GH1;
    strings.GATEWAY_HOST_GH2 = stored.GATEWAY_HOST_GH2;
    strings.GATEWAY_IP_GH1 = stored.GATEWAY_IP_GH1;
    strings.GATEWAY_IP_GH2 = stored.GATEWAY_IP_GH2;
    strings.ADMIN_PASSWORD = REDACTED
    strings.PORTAL_PASSWORD = REDACTED
  } else if (header_v2.version == 4) {
    StoredConfigV4 stored{};
    if (configFile.read((uint8_t*)&stored, sizeof(StoredConfigV4)) != sizeof(StoredConfigV4)) {
      configFile.close();
      return ConfigStatus::FILE_READ_ERROR;
    }
    configFile.close();
    ...
    if (header_v2.version >= 3) {
      uint32_t actual_crc = Crc32::compute(&stored, sizeof(StoredConfigV4));
      if (actual_crc != expected_crc) {
        return ConfigStatus::FILE_READ_ERROR;
      }
    }
    ...
    StoredConfigV3 stored{};
    if (configFile.read((uint8_t*)&stored, sizeof(StoredConfigV3)) != sizeof(StoredConfigV3)) {
      configFile.close();
      return ConfigStatus::FILE_READ_ERROR;
    }
    configFile.close();
    ...
    if (header_v2.version >= 3) {
      uint32_t actual_crc = Crc32::compute(&stored, sizeof(StoredConfigV3));
      if (actual_crc != expected_crc) {
        return ConfigStatus::FILE_READ_ERROR;
      }
    }
  ...
  if (header_v2.version < CONFIG_VERSION || stringsChanged) {
    ...
    const ConfigStatus saveStatus = save();  // migrate to latest format (adds CRC)
    if (saveStatus == ConfigStatus::OK) {
      releaseStrings();
    }
  } else {
...
ConfigStatus ConfigManager::save() {
...

  {
    ...
    temp_config.OTA_AUTH_TOKEN = REDACTED
    ...
    temp_config.GATEWAY_HOST_GH1 = m_strings->GATEWAY_HOST_GH1;
    temp_config.GATEWAY_HOST_GH2 = m_strings->GATEWAY_HOST_GH2;
    temp_config.GATEWAY_IP_GH1 = m_strings->GATEWAY_IP_GH1;
    temp_config.GATEWAY_IP_GH2 = m_strings->GATEWAY_IP_GH2;
    ...
    header.crc = Crc32::compute(&temp_config, sizeof(StoredConfig));
...
bool ConfigManager::ensureStringsLoaded() {
  ...
  if (LittleFS.exists(configFilePath)) {
    ...
    if (f) {
      ConfigFileHeaderV2 header_v2{};
      if (f.read((uint8_t*)&header_v2, sizeof(header_v2)) == sizeof(header_v2) && header_v2.magic == CONFIG_MAGIC) {
        uint32_t expected_crc = 0;
        if (header_v2.version >= 3) {
          ConfigFileHeaderV3 header_v3{};
          header_v3.magic = header_v2.magic;
        ...
        if (header_v2.version >= 5) {
          ...
          if (f.read((uint8_t*)&stored, sizeof(StoredConfig)) == sizeof(StoredConfig)) {
            if (header_v2.version >= 3) {
              ...
              if (actual_crc != expected_crc) {
                ...
                stored.OTA_AUTH_TOKEN = REDACTED
                ...
                stored.GATEWAY_HOST_GH1 = FactoryDefaults::STRINGS.GATEWAY_HOST_GH1;
                stored.GATEWAY_HOST_GH2 = FactoryDefaults::STRINGS.GATEWAY_HOST_GH2;
                stored.GATEWAY_IP_GH1 = FactoryDefaults::STRINGS.GATEWAY_IP_GH1;
                stored.GATEWAY_IP_GH2 = FactoryDefaults::STRINGS.GATEWAY_IP_GH2;
          ...
          StoredConfigV4 stored{};
          if (f.read((uint8_t*)&stored, sizeof(StoredConfigV4)) == sizeof(StoredConfigV4)) {
            if (header_v2.version >= 3) {
              uint32_t actual_crc = Crc32::compute(&stored, sizeof(StoredConfigV4));
              if (actual_crc != expected_crc) {
                stored.AUTH_TOKEN = REDACTED
          ...
          StoredConfigV3 stored{};
          if (f.read((uint8_t*)&stored, sizeof(StoredConfigV3)) == sizeof(StoredConfigV3)) {
            if (header_v2.version >= 3) {
              uint32_t actual_crc = Crc32::compute(&stored, sizeof(StoredConfigV3));
              if (actual_crc != expected_crc) {
                stored.AUTH_TOKEN = REDACTED
...
const char* ConfigManager::getOtaAuthToken() {
  (void)ensureStringsLoaded();
  if (!m_strings) {
    return "";
  }
  return (m_strings->OTA_AUTH_TOKEN[0] != REDACTED
}
const char* ConfigManager::getEffectiveUploadAuthToken() {
  (void)ensureStringsLoaded();
  if (!m_strings) {
    return "";
  }
  if (m_strings->AUTH_TOKEN[0] != REDACTED
    return m_strings->AUTH_TOKEN.data();
  }
  if (m_strings->OTA_AUTH_TOKEN[0] != REDACTED
    return m_strings->OTA_AUTH_TOKEN.data();
  }
...
const char* ConfigManager::getEffectiveOtaAuthToken() {
  (void)ensureStringsLoaded();
  if (!m_strings) {
    return "";
  }
...
const char* ConfigManager::getGatewayHost(uint8_t ghId) {
  (void)ensureStringsLoaded();
  if (!m_strings) {
    return "";
  }
  return (ghId == 2) ? m_strings->GATEWAY_HOST_GH2.data() : m_strings->GATEWAY_HOST_GH1.data();
}
const char* ConfigManager::getGatewayIp(uint8_t ghId) {
  (void)ensureStringsLoaded();
  if (!m_strings) {
    return "";
  }
  return (ghId == 2) ? m_strings->GATEWAY_IP_GH2.data() : m_strings->GATEWAY_IP_GH1.data();
}
...
bool ConfigManager::hasOtaTokenOverride() {
  (void)ensureStringsLoaded();
  return m_strings && m_strings->OTA_AUTH_TOKEN[0] != REDACTED
}
...
void ConfigManager::setOtaAuthToken(std::string_view token) {
  if (!ensureStringsLoaded())
    return;
  Utils::copy_string(m_strings->OTA_AUTH_TOKEN, token);
}
void ConfigManager::clearOtaAuthToken() {
  if (!ensureStringsLoaded())
    return;
  m_strings->OTA_AUTH_TOKEN[0] = REDACTED
}
void ConfigManager::setDataUploadUrl(std::string_view url) {
  if (!ensureStringsLoaded())
    return;
  Utils::copy_string(m_strings->DATA_UPLOAD_URL, url);
}
void ConfigManager::setOtaUrlBase(std::string_view url) {
  if (!ensureStringsLoaded())
    return;
  Utils::copy_string(m_strings->FW_VERSION_CHECK_URL_BASE, url);
}
void ConfigManager::setGatewayHost(uint8_t ghId, std::string_view host) {
  if (!ensureStringsLoaded())
    return;
  Utils::copy_string((ghId == 2) ? m_strings->GATEWAY_HOST_GH2 : m_strings->GATEWAY_HOST_GH1, host);
}
void ConfigManager::setGatewayIp(uint8_t ghId, std::string_view ip) {
  if (!ensureStringsLoaded())
    return;
  Utils::copy_string((ghId == 2) ? m_strings->GATEWAY_IP_GH2 : m_strings->GATEWAY_IP_GH1, ip);
}
...
void ConfigManager::setUplinkMode(UplinkMode mode) {
  m_config.set_uplink_mode(mode);
  validateAndSanitize();
}
...
void ConfigManager::validateAndSanitize() {
  ...
  if (m_config.flags.uplink_mode > static_cast<uint8_t>(UplinkMode::RELAY)) {
    m_config.set_uplink_mode(UplinkMode::AUTO);
  }
}

bool ConfigManager::validateAndSanitizeStrings(AppConfigStrings& strings) {
  bool changed = false;
  auto ensure_terminated = [&](auto& buf) {
    if (buf.back() != '\0') {
      buf.back() = '\0';
      changed = true;
    }
  ...
  ensure_terminated(strings.AUTH_TOKEN);
  ensure_terminated(strings.OTA_AUTH_TOKEN);
  ensure_terminated(strings.DATA_UPLOAD_URL);
  ensure_terminated(strings.FW_VERSION_CHECK_URL_BASE);
  ensure_terminated(strings.GATEWAY_HOST_GH1);
  ensure_terminated(strings.GATEWAY_HOST_GH2);
  ensure_terminated(strings.GATEWAY_IP_GH1);
  ensure_terminated(strings.GATEWAY_IP_GH2);
  ensure_terminated(strings.ADMIN_PASSWORD);
  ensure_terminated(strings.PORTAL_PASSWORD);

  if (!m_config.ALLOW_INSECURE_HTTPS()) {
    if (strings.DATA_UPLOAD_URL[0] != '\0' && !isValidHttpsUrl(strings.DATA_UPLOAD_URL.data())) {
      Utils::copy_string(strings.DATA_UPLOAD_URL, DEFAULT_DATA_URL);
      changed = true;
    }
    if (strings.FW_VERSION_CHECK_URL_BASE[0] != '\0' && !isValidHttpsUrl(strings.FW_VERSION_CHECK_URL_BASE.data())) {
      Utils::copy_string(strings.FW_VERSION_CHECK_URL_BASE, DEFAULT_OTA_URL_BASE);
      changed = true;
    }
  }

  // One-time migration: move legacy OTA default host to the new TA host.
  if (strings.FW_VERSION_CHECK_URL_BASE[0] != '\0' &&
      strncmp_P(strings.FW_VERSION_CHECK_URL_BASE.data(),
                PSTR("https://example.com/api/get-file/"),
                MAX_URL_LEN) == 0) {
    Utils::copy_string(strings.FW_VERSION_CHECK_URL_BASE, DEFAULT_OTA_URL_BASE);
    changed = true;
  }
  ...
  sanitize(std::span{strings.OTA_AUTH_TOKEN});
  ...
  sanitize(std::span{strings.GATEWAY_HOST_GH1});
  sanitize(std::span{strings.GATEWAY_HOST_GH2});
  sanitize(std::span{strings.GATEWAY_IP_GH1});
  sanitize(std::span{strings.GATEWAY_IP_GH2});
  return changed;
}
